...
namespace GEBUtils
{
    class RDPOperations
    {
        // Genera l'index dei file 
        public static void GenerateSeparatedIndex(bool debug)
        {
            try
            {
                if (debug)
                    Console.WriteLine("# Initializing package.rdp buffer");
                if (debug)
                {
                    Console.Write("# Indexing all files - 0%");
                }
                FileStream packagestream = new FileStream("package.rdp", FileMode.Open, FileAccess.Read);
                ...
                if (debug)
                {
                    Console.WriteLine("# Saving indexes file...");
                    Console.WriteLine("\n Files Statistics");
                }
                ...
                for (int i = 0; i < allPositions.Count; i++)
                {
                    ...
                    if (i < 5)
                    {
                        foreach (long position in allPositions[i])
                        {
                            ...
                            File.AppendAllText(fileName, position.ToString("X") + "|" + allDimensions[i][count].ToString("X") + "|" + prefixName + count.ToString() + "." + ext + "\r\n");
                            count++;
                        }
                    }
                    else
                    {
                        ...
                        {
                            File.AppendAllText(fileName, allPositions[i][count].ToString("X") + "|" + kvp.Value.ToString("X") + "|" + kvp.Key + "." + ext + "\r\n");
                            count++;
                        }
                    }
                    if (debug)
        ...
        public static void GenerateIndex(bool debug)
        {
            try
            {
                ...
                if (debug)
                    Console.Write("# Finding LBA free space - 0%");
                List<List<long>> allExtras = new List<List<long>>();
                int totalToCheck = allDimensions[0].Count + allDimensions[1].Count + allDimensions[2].Count + +allDimensions[3].Count;
                int totalChecked = 0;

                for (int i = 0; i < 5; i++)
                {
                    allExtras.Add(new List<long>());
                    if (i < 4)
                    {
                        int count = 0;
                        foreach (long position in allPositions[i])
                        {
                            packagestream.Seek(position + allDimensions[i][count], SeekOrigin.Begin);
                            byte tester = (byte)packagestream.ReadByte();
                            int testCount = 1;
                            while ((tester == (byte)packagestream.ReadByte()) && (testCount < 0x100000))
                            {
                                testCount++;
                                if (testCount == 0x1000000)
                                    break;
                            }
                            ...
                            allExtras[i].Add(testCount);
                            count++;
                            totalChecked++;

                            if (debug)
                                Console.Write("\r# Finding LBA free space - " + (int)(totalChecked * 100 / totalToCheck) + "%");
                        }
                    }
                }
                ...
                for (int i = 0; i < allPositions.Count; i++)
                {
                    ...
                    if (i < 5)
                    {
                        foreach (long position in allPositions[i])
                        {
                            ...
                            if (i == 4)
                            {
                                File.AppendAllText("index.lst", position.ToString("X") + "|" + allDimensions[i][count].ToString("X") + "|0|" + prefixName + count.ToString() + "." + ext + "\r\n");
                                count++;
                            }
                            else
                            {
                                File.AppendAllText("index.lst", position.ToString("X") + "|" + allDimensions[i][count].ToString("X") + "|" + allExtras[i][count].ToString("X") + "|" + prefixName + count.ToString() + "." + ext + "\r\n");
                                count++;
                            }
                        }
                    }
                    else
                    {
                        ...
                        if (i == 5)
                        {
                            foreach (KeyValuePair<string, long> kvp in tr2fileinfo)
                            {
                                File.AppendAllText("index.lst", allPositions[i][count].ToString("X") + "|" + kvp.Value.ToString("X") + "|0|" + kvp.Key + "." + ext + "\r\n");
                                count++;
                            }
                        }
                        else if (i == 6)
        ...
        public static string[] SubIndexType(string ext)
        {
            ...
            string[] allindexes = File.ReadAllLines("index.lst");
        ...
        public static void SingleUnpack(string args, bool debug)
        {
            ...
            foreach (string file in allfiles)
            {
                ...
                if (rawdata != "")
                {
                    string fileDir = Path.Combine("package", Path.GetExtension(file).Replace(".", ""));
        ...
        public static void UnpackSpecifiedTypes(FileStream packagestream, int filetype, bool debug)
        {
            ...
            fileDir = Path.Combine("package", ext);
        ...
        public static void Pack(string args, bool debug)
        {
            ...
            FileStream packagestream = new FileStream("package.rdp", FileMode.Open, FileAccess.Write);
        ...
        private static Dictionary<string, long> PRESHeadersAnalyzer(FileStream fs, List<long> positions)
        {
            ...
            foreach (long position in positions)
            {
                ...
                string fname = (string)SinglePRESHeaderAnalyzer(buffer, 0);
                long fend = (long)SinglePRESHeaderAnalyzer(buffer, 1);
                if (fend > 0)
                {
                    if (presFileInfo.ContainsKey(fname))
                    {
                        int x = 1;
                        while (presFileInfo.ContainsKey(fname + "_" + x))
                        {
                            x++;
                        }
                        presFileInfo.Add(fname + "_" + x, fend);
                    }
                    else
                    {
        ...
        private static object SinglePRESHeaderAnalyzer(byte[] rawdata, int mode)
        {
            ...
            long endName = endposition;

            while (rawdata[endName] != 0x00)
            {
                endName++;
            }

            byte[] rawpresname = new byte[endName - endposition - 1];
            Buffer.BlockCopy(rawdata, (int)endposition, rawpresname, 0, rawpresname.Length);
            string presName = Encoding.UTF8.GetString(rawpresname).Replace("\0", "");

            endposition += presName.Length;
